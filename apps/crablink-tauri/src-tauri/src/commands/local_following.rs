...
use crate::{
    local_following_store::{DesktopLocalFollowingStore, LocalFollowingRecordV1},
    state::AppState,
};
...
pub const LOCAL_FOLLOWING_READ_COMMAND: &str = "local_following_read";

pub const LOCAL_FOLLOWING_WRITE_COMMAND: &str = "local_following_write";

pub const LOCAL_FOLLOWING_UNAVAILABLE_MESSAGE: &str = "local following persistence unavailable";

pub const LOCAL_FOLLOWING_READ_FAILED_MESSAGE: &str = "local following read failed";

pub const LOCAL_FOLLOWING_WRITE_FAILED_MESSAGE: &str = "local following write failed";

pub fn read_local_following_from_store(
    store: &DesktopLocalFollowingStore,
) -> Result<Option<LocalFollowingRecordV1>, String> {
    store
        .load()
        .map_err(|_| LOCAL_FOLLOWING_READ_FAILED_MESSAGE.to_string())
}

pub fn write_local_following_to_store(
    store: &DesktopLocalFollowingStore,
    record: &LocalFollowingRecordV1,
) -> Result<LocalFollowingRecordV1, String> {
    store
        .write_atomic(record)
        .map_err(|_| LOCAL_FOLLOWING_WRITE_FAILED_MESSAGE.to_string())?;

    store
        .load()
        .map_err(|_| LOCAL_FOLLOWING_WRITE_FAILED_MESSAGE.to_string())?
        .ok_or_else(|| LOCAL_FOLLOWING_WRITE_FAILED_MESSAGE.to_string())
}
...
pub fn local_following_read(
    ...
    let store = state
        ...
        .as_ref()
        .ok_or_else(|| LOCAL_FOLLOWING_UNAVAILABLE_MESSAGE.to_string())?;

    let guard = store
        .lock()
        .map_err(|_| LOCAL_FOLLOWING_READ_FAILED_MESSAGE.to_string())?;

    read_local_following_from_store(&guard)
}
...
pub fn local_following_write(
    ...
    write_local_following_to_store(&guard, &record)
}
