...
const MAX_MAKE_EXPORT_AUDIO_TRACKS: usize = 8;
const MAX_MAKE_EXPORT_CHUNK_BYTES: usize = 512 * 1024;
const MAX_MAKE_EXPORT_AUDIO_CHUNK_BYTES: usize = MAX_MAKE_EXPORT_CHUNK_BYTES;
const MAX_MAKE_EXPORT_AUDIO_BYTES: u64 = 50 * 1024 * 1024;
...
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct MakeExportAppendAudioChunkInput {
    pub session_id: String,
...
pub struct MakeExportFinishInput {
    ...
    pub expected_audio_tracks: Option<usize>,
}
...
pub struct MakeExportStatus {
    ...
    pub uploaded_audio_tracks: usize,
...
pub struct MakeExportTruthBoundary {
    ...
    pub accepts_bounded_audio_chunks: bool,
    pub mixes_audio_tracks: bool,
...
pub struct MakeExportRecord {
    ...
    pub audio_tracks: Vec<MakeExportAudioTrack>,
    pub audio_mixed: bool,
    pub source: Option<VideoSourceRegistration>,
    pub warnings: Vec<String>,
    pub error: Option<MakeExportError>,
...
#[derive(Debug, Clone)]
#[allow(dead_code)]
pub struct MakeExportAudioTrack {
    pub id: String,
    pub file_name: String,
    pub mime_type: String,
    pub expected_bytes: u64,
...
pub fn begin_make_export_session(
    ...
    let mut warnings = vec![
        ...
        "Local audio lane tracks may be uploaded as bounded chunks and mixed by Rust/FFmpeg during finish.".to_string(),
...
pub fn append_make_export_audio_chunk(
    store: &MakeExportStore,
    input: MakeExportAppendAudioChunkInput,
) -> Result<MakeExportStatus, String> {
    validate_append_audio_input(&input)?;
    let clean_session_id = clean_handle(&input.session_id, "sessionId")?;

    let (audio_path, expected_offset) = {
        let mut guard = store
            .lock()
            .map_err(|_| "Make export store lock poisoned".to_string())?;
        let record = guard
        ...
        if record.status != "receiving" {
            return Err("Make export session is not accepting audio chunks".to_string());
        }

        let audio_track = ensure_audio_track(record, &input)?;
        (audio_track.path.clone(), audio_track.received_bytes)
    };

    if expected_offset != input.offset {
        return Err(format!(
            "audio chunk offset mismatch for track {}: expected {}, got {}",
            input.audio_track_id, expected_offset, input.offset
        ));
    }

    {
        let mut file = OpenOptions::new()
            ...
            .append(true)
            .open(&audio_path)
            .map_err(|_| "could not open Make export audio track for append".to_string())?;

        file.write_all(&input.bytes)
            .map_err(|_| "could not write Make export audio chunk".to_string())?;
        file.flush()
            .map_err(|_| "could not flush Make export audio chunk".to_string())?;
    }

    {
        ...
        let clean_audio_id = clean_handle(&input.audio_track_id, "audioTrackId")?;
        let audio_track = record
            .audio_tracks
            .iter_mut()
            .find(|candidate| candidate.id == clean_audio_id)
            .ok_or_else(|| "Make export audio track disappeared".to_string())?;

        audio_track.received_bytes = audio_track
            .received_bytes
            .checked_add(input.bytes.len() as u64)
            .ok_or_else(|| "Make export audio byte count overflow".to_string())?;
        record.received_audio_bytes = record
            .received_audio_bytes
            .checked_add(input.bytes.len() as u64)
            .ok_or_else(|| "Make export audio byte count overflow".to_string())?;

        if audio_track.received_bytes > audio_track.expected_bytes {
            return Err("Make export audio track received more bytes than declared".to_string());
        }

        if record.received_audio_bytes > MAX_MAKE_EXPORT_AUDIO_BYTES {
            return Err(format!(
                "Make export audio exceeds local audio cap of {} bytes",
                MAX_MAKE_EXPORT_AUDIO_BYTES
            ));
        }

        record.updated_at_unix_ms = now_unix_ms()?;
        record.phase = format!("receiving_audio_track_{}", record.audio_tracks.len());
        record.progress_percent = receiving_progress(record);
    }
...
pub fn finish_make_export_session(
    ...
    let (dir, output_path, fps, width, height, title, output_file_name, segments, audio_tracks) = {
        let mut guard = store
            .lock()
            .map_err(|_| "Make export store lock poisoned".to_string())?;
        let record = guard
        ...
        for audio_track in &record.audio_tracks {
            if audio_track.received_bytes != audio_track.expected_bytes {
                return Err(format!(
                    "Make export audio track {} is incomplete",
                    audio_track.id
                ));
            }
        }

        if let Some(expected_audio_tracks) = input.expected_audio_tracks {
            let uploaded_unmuted = record
                .audio_tracks
                .iter()
                .filter(|track| !track.muted && track.volume_pct > 0)
                .count();
            if uploaded_unmuted < expected_audio_tracks {
                return Err(format!(
                    "Make export expected {} audio track(s), but only {} unmuted local track(s) were uploaded",
                    expected_audio_tracks, uploaded_unmuted
                ));
            }
        }

        record.status = "transcoding".to_string();
        record.phase = if record.audio_tracks.is_empty() {
            "normalizing_clip_segments".to_string()
        } else {
            "normalizing_clip_segments_before_audio_mix".to_string()
        };
        ...
        let mut audio_tracks = record.audio_tracks.clone();
        audio_tracks.sort_by(|left, right| left.id.cmp(&right.id));
    ...
    let ready_audio_tracks: Vec<MakeExportAudioTrack> = audio_tracks
        .iter()
        .filter(|track| !track.muted && track.volume_pct > 0)
        .cloned()
        .collect();
    let audio_mix_requested = !ready_audio_tracks.is_empty();
    let base_video_path = if audio_mix_requested {
        dir.join("video-before-audio-mix.mp4")
    } else {
        output_path.clone()
    };

    run_ffmpeg_concat_normalized(&concat_list_path, &base_video_path)?;

    if audio_mix_requested {
        {
            let mut guard = store
                .lock()
                .map_err(|_| "Make export store lock poisoned".to_string())?;
            if let Some(record) = guard.get_mut(&clean_session_id) {
                record.phase = "mixing_audio_lane_into_mp4".to_string();
                record.progress_percent = 97;
                record.updated_at_unix_ms = now_unix_ms()?;
            }
        }

        run_ffmpeg_mix_audio_tracks(&base_video_path, &ready_audio_tracks, &output_path)?;
    }
    ...
    let total_input_bytes: u64 = segments.iter().map(|segment| segment.expected_bytes).sum();

    if output_bytes < 16 * 1024 {
        return Err(format!(
            "Make export produced an implausibly tiny MP4: output={} bytes from input={} bytes.",
    ...
    if !has_video_stream(&output_path) {
        return Err(format!(
            "Make export produced an MP4 without a readable video stream: output={} bytes from input={} bytes.",
            output_bytes, total_input_bytes
        ));
    }

    if audio_mix_requested && !has_audio_stream(&output_path) {
        return Err(
            "Make export audio mix failed: expected audio output but no audio stream was found."
                .to_string(),
        );
    }
...

    {
        ...
        record.phase = if audio_mix_requested {
            "mp4_source_handle_ready_with_audio_mix".to_string()
        } else {
            "mp4_source_handle_ready".to_string()
        };
        ...
        record.audio_mixed = audio_mix_requested;
        ...
        if audio_mix_requested {
            record.warnings.push(
                "Local audio lane tracks were mixed into this MP4 by Rust/FFmpeg before video-page handoff."
                    .to_string(),
            );
        }
    }
...
fn validate_append_audio_input(input: &MakeExportAppendAudioChunkInput) -> Result<(), String> {
    if input.bytes.is_empty() {
        return Err("Make export audio chunk is empty".to_string());
    }

    if input.bytes.len() > MAX_MAKE_EXPORT_AUDIO_CHUNK_BYTES {
        return Err(format!(
            "Make export audio chunk exceeds {} byte command cap",
            MAX_MAKE_EXPORT_AUDIO_CHUNK_BYTES
        ));
    }

    if input.total_bytes == 0 {
        return Err("Make export audio totalBytes is required".to_string());
    }

    if input.total_bytes > MAX_MAKE_EXPORT_AUDIO_BYTES {
        return Err(format!(
            "Make export audio track exceeds local audio cap of {} bytes",
            MAX_MAKE_EXPORT_AUDIO_BYTES
        ));
    }
    ...
    let end = input
        .offset
        .checked_add(chunk_len)
        .ok_or_else(|| "Make export audio chunk offset overflow".to_string())?;

    if end > input.total_bytes {
        return Err("Make export audio chunk extends past declared track length".to_string());
    }
    ...
    let mime = input
        ...
        .trim()
        .to_ascii_lowercase();
    if !mime.is_empty() && mime != "audio/*" && !mime.starts_with("audio/") {
        return Err("Make export audio upload expects an audio/* MIME type".to_string());
    }
...
fn ensure_audio_track<'a>(
    record: &'a mut MakeExportRecord,
    input: &MakeExportAppendAudioChunkInput,
) -> Result<&'a mut MakeExportAudioTrack, String> {
    let clean_audio_id = clean_handle(&input.audio_track_id, "audioTrackId")?;

    if let Some(position) = record
        .audio_tracks
        .iter()
        .position(|candidate| candidate.id == clean_audio_id)
    {
        let audio_track = record
            .audio_tracks
            .get_mut(position)
            .ok_or_else(|| "Make export audio track lookup failed".to_string())?;

        if audio_track.expected_bytes != input.total_bytes {
            return Err("audio totalBytes changed after upload started".to_string());
        }

        return Ok(audio_track);
    }

    if record.audio_tracks.len() >= MAX_MAKE_EXPORT_AUDIO_TRACKS {
        return Err(format!(
            "Make export supports at most {MAX_MAKE_EXPORT_AUDIO_TRACKS} local audio tracks"
        ));
    }

    let file_name = safe_audio_file_name(
        input.track_name.as_deref().unwrap_or("make-audio-track"),
        &clean_audio_id,
        input.mime_type.as_deref().unwrap_or("audio/*"),
    );
    let path = record.dir.join("audio").join(&file_name);

    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|_| "could not create Make export audio staging directory".to_string())?;
    }
    ...
    let trim_start_ms = input.trim_start_ms.unwrap_or(0);
    let trim_end_ms = input.trim_end_ms.filter(|value| *value > trim_start_ms);

    record.audio_tracks.push(MakeExportAudioTrack {
        id: clean_audio_id.clone(),
        file_name,
        mime_type: input
            .mime_type
            .clone()
        ...
        received_bytes: 0,
        path,
        timeline_offset_ms: input.timeline_offset_ms.unwrap_or(0),
        trim_start_ms,
        trim_end_ms,
    ...
    record
        .audio_tracks
        .iter_mut()
        .find(|track| track.id == clean_audio_id)
        .ok_or_else(|| "Make export audio track insert failed".to_string())
}
...
fn run_ffmpeg_normalize_segment(
    ...
    let input_has_audio = has_audio_stream(input_path);
...
fn run_ffmpeg_mix_audio_tracks(
    base_video_path: &Path,
    audio_tracks: &[MakeExportAudioTrack],
    output_path: &Path,
) -> Result<(), String> {
    if audio_tracks.is_empty() {
        return Err("Make export audio mix requested with no audio tracks".to_string());
    }
    ...
    for track in audio_tracks {
        if !track.path.exists() {
            return Err("Make export audio track missing on disk before mix".to_string());
        }
        command.arg("-i").arg(&track.path);
    }

    let base_has_audio = has_audio_stream(base_video_path);
    ...
    for (index, track) in audio_tracks.iter().enumerate() {
        ...
        filter_parts.push(format!(
            "[{input_index}:a:0]{trim},asetpts=PTS-STARTPTS,aresample=48000,aformat=sample_rates=48000:channel_layouts=stereo,volume={volume:.3},adelay={delay}|{delay}[{label}]",
            delay = track.timeline_offset_ms,
        ));
        mix_inputs.push(format!("[{label}]"));
    }

    if mix_inputs.is_empty() {
        return Err("Make export audio mix found no usable audio inputs".to_string());
    }

    if mix_inputs.len() == 1 {
        filter_parts.push(format!("{}anull[mix]", mix_inputs[0]));
    } else {
        filter_parts.push(format!(
            "{}amix=inputs={}:duration=first:dropout_transition=0,alimiter=limit=0.98[mix]",
            mix_inputs.join(""),
            mix_inputs.len()
        ));
    }
    ...
    run_command(command, "FFmpeg Make audio lane mix failed")
}
...
fn has_video_stream(path: &Path) -> bool {
    ...
    let output = Command::new(ffprobe)
        ...
        .arg("v:0")
        .arg("-show_entries")
        .arg("stream=width,height,duration,nb_frames")
        .arg("-of")
        .arg("csv=p=0")
        .arg(path)
        .output();
...
fn status_from_record(record: &MakeExportRecord) -> MakeExportStatus {
    MakeExportStatus {
        ...
        audio_track_count: record.audio_tracks.len(),
        uploaded_audio_tracks: record
            .audio_tracks
            .iter()
            .filter(|track| track.received_bytes == track.expected_bytes)
            .count(),
        received_audio_bytes: record.received_audio_bytes,
        audio_mixed: record.audio_mixed,
        source: record.source.clone(),
        warnings: record.warnings.clone(),
        error: record.error.clone(),
        truth_boundary: MakeExportTruthBoundary {
            accepts_bounded_chunks: true,
            accepts_bounded_audio_chunks: true,
            mixes_audio_tracks: record.audio_mixed,
...
fn safe_audio_file_name(value: &str, audio_track_id: &str, mime_type: &str) -> String {
    let stem = sanitize_file_stem(value);
    let id = clean_handle(audio_track_id, "audioTrackId").unwrap_or_else(|_| "audio".to_string());
    format!(
        "audio-{}-{}.{}",
        id,
        if stem.is_empty() { "track" } else { &stem },
        audio_extension(value, mime_type)
    )
}

fn audio_extension(value: &str, mime_type: &str) -> &'static str {
    let lower = value.to_lowercase();
...
fn ms_to_ffmpeg_seconds(ms: u64) -> String {
