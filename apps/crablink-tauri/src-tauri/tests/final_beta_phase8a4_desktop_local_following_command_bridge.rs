...
fn phase8a4_corrupt_disk_state_returns_generic_read_failure() {
    ...
    fs::write(
        store.record_path(),
        b"corrupt local following bytes",
    )
    .expect(
        "write corrupt fixture",
    );
...
fn phase8a4_app_state_and_desktop_setup_wire_one_serialized_store() {
    ...
    let lib =
        ...
        .expect(
            "read Tauri lib source",
        );

    assert!(
        state.contains(
            "Option<Mutex<DesktopLocalFollowingStore>>",
        ),
    );

    assert!(
        state.contains(
            "with_native_passport_runtime_and_local_following",
        ),
    );

    assert!(
        lib.contains(
            "initialize_desktop_local_following_store(&app_data_directory)",
        ),
    );

    assert!(
        lib.contains(
            "AppState::with_native_passport_runtime_and_local_following",
        ),
    );
}
...
fn phase8a4_command_bridge_adds_no_social_graph_or_economic_authority() {
    ...
    let source =
        fs::read_to_string(
            root.join(
                "src/commands/local_following.rs",
            ),
        )
        .expect(
            "read local following command source",
        );
