...
use crablink_tauri_lib::local_following_store::{
    initialize_desktop_local_following_store,
    ...
    fn new(
        ...
        let timestamp =
            SystemTime::now()
                ...
                .expect(
                    "test clock after Unix epoch",
                )
                .as_nanos();

        let counter =
            TEST_DIRECTORY_COUNTER
                .fetch_add(
                    1,
                    Ordering::Relaxed,
                );
...
fn phase8a3_empty_record_round_trips_atomically() {
    ...
    store
        ...
        .expect(
            "write empty local following record",
        );
...
fn phase8a3_strict_record_validation_rejects_schema_duplicates_and_profile_mismatch() {
    ...
    mismatch_entry.profile_ref =
        "crab://@differentcreator"
            .to_string();
...
fn phase8a3_validation_rejects_invalid_username_timestamp_and_cursor() {
    ...
    invalid_cursor.last_timeline_cursor =
        Some(
            "x".repeat(
                513,
            ),
...
fn phase8a3_entry_count_is_bounded() {
    let mut entries =
        Vec::with_capacity(
            LOCAL_FOLLOWING_MAX_ENTRIES + 1,
        );

    for index in
        0..=LOCAL_FOLLOWING_MAX_ENTRIES
    {
        let username =
            format!(
                "u{index:05}",
...
fn phase8a3_oversized_disk_record_is_rejected_before_decode() {
    ...
    let file =
        ...
        .expect(
            "create oversized fixture",
        );

    file.set_len(
        LOCAL_FOLLOWING_MAX_FILE_BYTES + 1,
    )
    .expect(
        "set oversized fixture length",
    );
...
fn phase8a3_native_store_adds_no_network_or_economic_authority() {
    ...
    let source =
        fs::read_to_string(
            root.join(
                "src/local_following_store.rs",
            ),
        )
        .expect(
            "read local following store source",
        );
