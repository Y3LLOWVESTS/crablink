 ...
 * RO:WHY — Completes the frontend text primitive contract without faking backend truth; Tauri uses in-page review/send instead of window.confirm.
 ...
 * RO:INVARIANTS — no silent ROC spend; hold requires explicit review/send; publish requires backend hold proof; no direct storage/index/ledger calls.
 ...
 * RO:TEST — manual crab://article prepare/hold/publish smoke from Tauri React shell after backend article routes exist.
 */
...
export default function ArticlePublishFlow({ app, draftState }) {
  ...
  async function submitPublish() {
    ...
    if (!publishReviewOpen) {
      ...
      app?.notify?.({
        title: 'Review article publish request',
        message: 'Review the article publish request, then click Send Article Publish. No article JSON has been sent yet.',
        tone: 'info',
      });
  ...
  return (
    ...
    >
      <p className="article-section-copy">
        This is the gateway-wired lane for the article primitive. It can call the expected
      ...
      <section className="article-publish-step">
        ...
        {holdReviewOpen && holdApiRequest && (
          <div className="article-publish-warning" role="status" aria-live="polite">
            <strong>Review ROC hold for article publish</strong>
            <span>
      ...
      <section className="article-publish-step">
        <header>
          <div>
            ...
            <h3>Review and submit article publish</h3>
          </div>
        ...
        {publishReviewOpen && (
          <div className="article-publish-warning" role="status" aria-live="polite">
            <strong>Review article publish request</strong>
            <span>
              No article JSON has been sent yet. Click <strong>Send Article Publish</strong> to submit the article and paid proof.
            </span>
          </div>
        )}

        <div className="article-publish-actions">
          <Button variant={publishReviewOpen ? 'primary' : 'secondary'} disabled={!canPublish || publishState.status === 'sending'} onClick={submitPublish}>
            {publishState.status === 'sending' ? 'Publishing…' : publishReviewOpen ? 'Send Article Publish' : 'Review Article Publish'}
          </Button>
          {publishReviewOpen && (
            <Button variant="secondary" disabled={publishState.status === 'sending'} onClick={() => setPublishReviewOpen(false)}>
              Cancel Review
            </Button>
          )}
        ...
        <JsonPreview
          ...
          data={{
            ...
            pendingRequest: publishReviewOpen
              ? {
                  route: '/assets/article',
                  publishPayload,
                  paidProof,
          ...
          initiallyOpen={publishReviewOpen || publishState.status === 'error' || publishState.status === 'ok'}
        />
      </section>
    </Card>
  );
