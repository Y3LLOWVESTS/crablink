...
import VideoAssetManifestFacts from './VideoAssetManifestFacts.jsx';
import { readVideoBundleRenditionGroupForAsset } from '../../shared/manifest/renditionGroups.js';
...
export default function AssetHydratedView({ route, app, result, assetClient, resolverState }) {
  ...
  const summary = useMemo(() => withLocalVideoBundleFallback(summarizeAsset(result, route)), [result, route]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      for (const routeCandidate of routes) {
        try {
          const response = await fetchPaidVideoBytes({
            route: routeCandidate.route,
            summary,
            assetClient,
          });

          const blob = await normalizeVideoBytesResponse(response, summary.contentType);
    ...
    <section className="asset-hydrated-view">
      <section className="asset-overview-grid">
          ...
          {summary.isImageRoute ? (
            ...
            <VideoAssetManifestFacts
              ...
              fallbackText={
                summary.description ||
                textContent.summary?.bodyPreview ||
                'The gateway returned this typed video response without a public description field.'
              }
            />
          ) : (
        ...
        <aside className="asset-side-panel" aria-label="Asset summary stats">
          ...
          {(summary.isImageRoute || summary.isVideoRoute) && summary.renditionCount > 0 && (
      ...
      {summary.isVideoRoute && summary.renditionGroup?.renditions?.length > 0 && (
        <VideoRenditionGroupCard summary={summary} app={app} />
      )}
...
function ImageRenditionGroupCard({ summary, app }) {
  ...
  const isVideoBundle = String(group.schema || group.raw?.schema || '').includes('video') ||
    renditions.some((item) => item.assetKind === 'video');
  ...
  return (
    <Card
      eyebrow={isVideoBundle ? 'Bundle versions' : 'Image versions'}
      title={isVideoBundle ? 'Video rendition group' : 'Rendition group'}
    ...
    >
      <p className="asset-description">
        {isVideoBundle
          ? 'This structured block is the sibling map for the video bundle. Every row is still its own backend-returned b3 asset; the group is reference metadata, not a replacement for canonical b3 truth.'
          : 'This structured block is the sibling map for the image bundle. Every row is still its own backend-returned b3 image asset; the group is reference metadata, not a replacement for canonical b3 truth.'}
      </p>
      ...
      <div className="asset-rendition-table" aria-label="Image rendition group rows">
        ...
        {renditions.map((item, index) => {
          ...
          const typeLabel = [item.assetKind ? labelFromKind(item.assetKind) : '', item.mime || 'Type n/a'].filter(Boolean).join(' · ');
          const rowTitle = [
...
function VideoRenditionGroupCard({ summary, app }) {
  const group = summary.renditionGroup || {};
  const renditions = safeArray(group.renditions);
  ...
  return (
    <Card
      eyebrow="Video versions"
      title="Rendition group"
      className="asset-rendition-card asset-video-rendition-card"
      actions={
        <div className="asset-copy-actions">
          <CopyButton text={group.canonicalCrabUrl || summary.crabUrl || ''} label="Copy primary URL" />
          <CopyButton text={group.sourceCid || summary.cid || ''} label="Copy source CID" />
        </div>
      }
    >
      <p className="asset-description">
        This is the sibling map for the minted video bundle. Every row is still its own backend-returned b3 asset;
        this table only records the relationship between the video versions, poster image, and thumbnail image.
      </p>

      <div className="asset-fact-grid asset-rendition-facts">
        <Fact label="Group ID" value={group.groupId || 'Not returned'} monospace />
        <Fact label="Source CID" value={group.sourceCid || 'Not returned'} monospace />
        <Fact label="Canonical primary" value={group.canonicalCrabUrl || 'Not returned'} monospace />
        <Fact label="Current role" value={summary.rendition?.label || summary.rendition?.role || 'Not returned'} />
      </div>

      <div className="asset-rendition-table" aria-label="Video rendition group rows">
        <div className="asset-rendition-head" aria-hidden="true">
          <span>Type</span>
          <span>Crab URL / CID</span>
          <span>Asset facts</span>
        ...
        {renditions.map((item, index) => {
          const dimensions = item.width || item.height ? `${item.width || 0}×${item.height || 0}` : 'Dimensions n/a';
          const byteLabel = item.bytes ? formatBytes(item.bytes) : 'Size n/a';
          const typeLabel = [labelFromKind(item.assetKind || 'asset'), item.mime || 'Type n/a'].filter(Boolean).join(' · ');
          const rowTitle = [
            item.label || labelFromKind(item.role || 'rendition'),
            item.isCurrent ? 'current' : '',
            item.isOriginal ? 'primary' : '',
          ].filter(Boolean).join(' · ');
          ...
          return (
            ...
            >
              <div className="asset-rendition-role">
                <Badge tone={item.isCurrent ? 'success' : item.assetKind === 'image' ? 'info' : 'neutral'}>
                  {item.label || labelFromKind(item.role || 'rendition')}
                </Badge>
                <small>{rowTitle}</small>
              </div>
              ...
              <div className="asset-rendition-actions">
                <Button type="button" variant="secondary" size="sm" disabled={!item.crabUrl} onClick={() => openCrabUrl(item.crabUrl)}>
                  Open
                </Button>
                {item.crabUrl && <CopyButton text={item.crabUrl} label="Copy URL" />}
                {item.cid && <CopyButton text={item.cid} label="Copy CID" />}
      ...
      <details className="asset-preview-fallbacks asset-rendition-json">
        ...
        <JsonPreview label="video_rendition_group" data={group.raw || group} initiallyOpen={false} />
      </details>
    </Card>
  );
}
...
function summarizeAsset(result, route) {
  ...
  const rawRenditionGroup =
    data.rendition_group ||
    data.renditionGroup ||
    manifest.rendition_group ||
    manifest.renditionGroup ||
    metadata.rendition_group ||
    metadata.renditionGroup ||
    metadata.video_rendition_group ||
    metadata.videoRenditionGroup;
  const renditionGroup = kind === 'video'
    ? normalizeVideoRenditionGroup(rawRenditionGroup, cid)
    : normalizeImageRenditionGroup(rawRenditionGroup, cid);
  const rendition = kind === 'video'
    ? normalizeVideoRenditionSelf(
        data.rendition || data.video_rendition || data.videoRendition || manifest.rendition,
        renditionGroup,
        cid,
      )
    : normalizeImageRenditionSelf(
...
function withLocalVideoBundleFallback(summary) {
  if (!summary?.isVideoRoute && !summary?.isImageRoute) {
    return summary;
  }

  const cachedGroup = readVideoBundleRenditionGroupForAsset({
    cid: summary.cid,
  ...
  const normalizedCachedGroup = normalizeVideoRenditionGroup(cachedGroup, summary.cid)
    || normalizeImageRenditionGroup(cachedGroup, summary.cid);

  if (!normalizedCachedGroup?.renditions?.length) {
    return summary;
  }

  const currentCount = summary.renditionGroup?.renditions?.length || 0;
  if (currentCount >= normalizedCachedGroup.renditions.length) {
    return summary;
  }

  const normalizedRendition = summary.isVideoRoute
    ? normalizeVideoRenditionSelf(summary.rendition, normalizedCachedGroup, summary.cid)
    : normalizeImageRenditionSelf(summary.rendition, normalizedCachedGroup, summary.cid);

  return {
    ...summary,
    renditionGroup: normalizedCachedGroup,
    rendition: normalizedRendition || summary.rendition,
    renditionCount: normalizedCachedGroup.renditions.length,
    localDisplayBundle: true,
  };
}
...
function normalizeImageRenditionItem(value, group = {}, selfCid = '') {
  ...
  return {
    ...
    assetKind: cleanKind(raw.asset_kind || raw.assetKind || group.asset_kind || group.assetKind || 'image'),
    cid,
...
function normalizeVideoRenditionGroup(value, selfCid = '') {
  const raw = objectValue(value);
  const items = safeArray(raw.renditions)
    .map((item) => normalizeVideoRenditionItem(item, raw, selfCid))
    .filter(Boolean);
  ...
  const sourceCid = cleanCid(
    raw.source_cid ||
      ...
      raw.source ||
      items.find((item) => item.isOriginal)?.cid ||
      items.find((item) => item.assetKind === 'video')?.cid ||
      items[0]?.cid,
  );
  const canonicalCrabUrl = stringValue(
    raw.canonical_crab_url,
    ...
    raw.canonicalOriginal,
    raw.original,
    sourceCid ? `crab://${sourceCid.replace(/^b3:/, '')}.video` : '',
  );

  return {
    schema: stringValue(raw.schema, 'crablink.video-rendition-group.v1'),
    groupId: cleanCid(raw.group_id || raw.groupId || raw.id || sourceCid) || stringValue(raw.group_id, raw.groupId, raw.id),
    sourceCid,
    canonicalCrabUrl,
    renditions: items.map((item) => ({
...
function normalizeVideoRenditionItem(value, group = {}, selfCid = '') {
  const raw = objectValue(value);
  const assetKind = cleanKind(raw.asset_kind || raw.assetKind || raw.kind || 'video') || 'video';
  const cid = cleanCid(raw.cid || raw.asset_cid || raw.assetCid || raw.content_id || raw.contentId);
  ...
  const groupSourceCid = cleanCid(group.source_cid || group.sourceCid || group.original_cid || group.originalCid);
  const role = stringValue(raw.role, cid === groupSourceCid ? 'source_clean_master' : 'rendition');
  const crabUrl = stringValue(raw.crab_url, raw.crabUrl, raw.url, `crab://${cid.replace(/^b3:/, '')}.${assetKind}`);
  const isOriginal = Boolean(
    raw.is_original ||
      raw.isOriginal ||
      role === 'source_clean_master' ||
      role === 'original' ||
      (groupSourceCid && cid === groupSourceCid),
  );

  return {
    role,
    label: stringValue(raw.label, videoRenditionLabel(role)),
    assetKind,
    cid,
    hash: normalizeHash(raw.hash || cid),
    crabUrl,
...
function normalizeVideoRenditionSelf(value, group, selfCid = '') {
  const groupMatch = group?.renditions?.find((item) => item.cid === selfCid);
  ...
  const sourceCid = cleanCid(group?.sourceCid || raw.derived_from || raw.derivedFrom || cid);
  const role = stringValue(raw.role, cid === sourceCid ? 'source_clean_master' : 'rendition');
  const assetKind = cleanKind(raw.asset_kind || raw.assetKind || 'video') || 'video';
  ...
  return {
    ...
    assetKind,
    cid,
    crabUrl: stringValue(raw.crab_url, raw.crabUrl, raw.url, `crab://${cid.replace(/^b3:/, '')}.${assetKind}`),
    mime: stringValue(raw.mime, raw.content_type, raw.contentType),
    bytes: numberValue(raw.bytes, raw.byte_length, raw.byteLength),
    width: numberValue(raw.width),
    height: numberValue(raw.height),
    isOriginal: cid === sourceCid || role === 'source_clean_master' || role === 'original',
    isCurrent: true,
    derivedFrom: cid === sourceCid ? '' : sourceCid,
    canonicalOriginal: stringValue(raw.canonical_original, raw.canonicalOriginal, group?.canonicalCrabUrl),
    raw,
...
function videoRenditionLabel(role) {
  const map = {
  ...
  return map[role] || labelFromKind(role || 'rendition');
}
...
function audioModeLabel(status) {
  switch (status) {
    case 'loading':
      return canUseTauriInvoke() ? 'Tauri byte bridge loading' : 'paid blob fetch loading';
    case 'ready':
      return canUseTauriInvoke() ? 'Tauri blob playback' : 'paid blob playback';
    case 'error':
...
async function fetchPaidVideoBytes({ route, summary = {}, assetClient }) {
  const objectRoute = canonicalVideoObjectRoute(route, summary);
  ...
  if (canUseTauriInvoke()) {
    ...
    const response = await callTauri('fetch_asset_bytes_gateway', {
      request: {
        ...
        contentTypeHint: inferVideoContentType(summary?.contentType),
        maxBytes: MAX_VIDEO_BLOB_PREVIEW_BYTES,
        headers: {
          'x-ron-asset-kind': summary?.kind || 'video',
        },
      },
    });
  ...
  if (!assetClient?.gateway?.request) {
    throw new Error('Gateway client is unavailable for video preview.');
  }
...
async function normalizeVideoBytesResponse(response, contentType) {
  if (response?.blob) {
    return normalizeVideoBlobResponse(response.blob, contentType);
  }

  const bytes = normalizeByteList(response?.bodyBytes || response?.body_bytes);

  if (!bytes.length) {
    throw new Error('Gateway returned empty video bytes.');
  }

  const blob = new Blob([bytes], {
    type: response?.contentType || response?.content_type || inferVideoContentType(contentType),
  });

  return normalizeVideoBlobResponse(blob, contentType);
}

function canonicalVideoObjectRoute(route, summary = {}) {
  const raw = normalizeGatewayRoute(route);
  const objectMatch = raw.match(/^\/o\/b3:([0-9a-f]{64})$/i);
...
async function normalizeVideoBlobResponse(blob, contentType) {
  ...
  const returnedType = String(blob.type || '').toLowerCase();
  const bytes = await blob.arrayBuffer();
  const sniffedType = sniffVideoContentType(bytes);
  ...
  return new Blob([bytes], {
    type: sniffedType || inferVideoContentType(contentType),
  });
}
