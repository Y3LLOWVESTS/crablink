...
function LibraryCard({ item, app }) {
  ...
  return (
    <article className={`cl-library-card is-${kind} is-${item.type || 'entry'}`}>
      <header>
        <div>
      ...
      <footer className="cl-library-card-actions">
        {item.type === 'receipt' && (
          <Button variant="secondary" onClick={() => app?.navigate?.('crab://receipts')} disabled={!app?.navigate}>
            Receipt page
          </Button>
        )}
  ...
  return safeArray(receipts)
    .map((item) => {
      ...
      return {
        ...
        title: item.title || labelForReceipt(action),
        crabUrl: route,
        route,
        ...
        receiptHash,
        txid,
        ledgerRoot: stringValue(item.ledgerRoot, item.ledger_root),
        nonce: stringValue(item.nonce),
        idempotencyKey: stringValue(item.idempotencyKey, item.idempotency_key, item.idem),
        amountDisplay: formatReceiptAmount(amount, item.asset),
        payer,
        recipient,
        action,
        status: item.status || 'backend receipt display cache',
        detail: [formatReceiptAmount(amount, item.asset), payer && recipient ? `${payer} → ${recipient}` : '']
          .filter(Boolean)
          .join(' · '),
        source: item.source || 'recent_receipts',
        createdAt: item.createdAt || item.created_at || item.storedAt || item.paidAt || item.timestamp || '',
        raw: item.raw || item,
      };
    })
...
function labelForReceipt(action) {
  const clean = String(action || '').replace(/[_-]+/g, ' ').trim();

  if (clean) {
    return labelFromKind(clean);
  }

  return 'Receipt';
}

function formatCardTitle(item, route, proof) {
  if (item?.type === 'receipt') {
    return item.title || labelForReceipt(item.action || item.kind);
  }

  return item?.title || item?.name || route || proof || 'Local entry';
}

function receiptPaymentLine(item) {
  return [item.amountDisplay, item.payer && item.recipient ? `${item.payer} → ${item.recipient}` : '']
    .filter(Boolean)
    .join(' · ') || 'not returned';
}

function formatReceiptAmount(amount, asset = 'roc') {
  ...
  if (/\bROC$/i.test(clean)) {
    return clean.replace(/\s+ROC$/i, ' ROC');
  }

  return `${clean} ${String(asset || 'roc').toUpperCase()}`;
}
