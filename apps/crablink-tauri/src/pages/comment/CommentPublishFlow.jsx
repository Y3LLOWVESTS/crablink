 ...
 * RO:WHY — Adds the comment asset publishing lane without faking backend truth; Tauri uses in-page review/send instead of window.confirm.
 ...
 * RO:TEST — manual crab://comment prepare/hold/publish smoke from Tauri React shell after backend comment routes exist.
 */
...
export default function CommentPublishFlow({ app, draftState }) {
  ...
  async function submitPublish() {
    ...
    if (!publishReviewOpen) {
      ...
      app?.notify?.({
        title: 'Review comment publish request',
        message: 'Review the comment publish request, then click Send Comment Publish. No comment JSON has been sent yet.',
        tone: 'info',
      });
  ...
  return (
    ...
    >
      <p className="comment-section-copy">
        This is the gateway-wired lane for the comment primitive. It can call the expected
      ...
      <section className="comment-publish-step">
        ...
        {holdReviewOpen && holdApiRequest && (
          <div className="comment-publish-warning" role="status" aria-live="polite">
            <strong>Review ROC hold for comment publish</strong>
            <span>
      ...
      <section className="comment-publish-step">
        <header>
          <div>
            ...
            <h3>Review and submit comment publish</h3>
          </div>
        ...
        {publishReviewOpen && (
          <div className="comment-publish-warning" role="status" aria-live="polite">
            <strong>Review comment publish request</strong>
            <span>
              No comment JSON has been sent yet. Click <strong>Send Comment Publish</strong> to submit the comment and paid proof.
            </span>
          </div>
        )}
        ...
        <div className="comment-publish-actions">
          <Button variant={publishReviewOpen ? 'primary' : 'secondary'} disabled={!canPublish || publishState.status === 'sending'} onClick={submitPublish}>
            {publishState.status === 'sending' ? 'Publishing…' : publishReviewOpen ? 'Send Comment Publish' : 'Review Comment Publish'}
          </Button>
          {publishReviewOpen && (
