 ...
 * RO:TEST — manual crab://site guided launch smoke from Tauri React button.
 */
...
export default function SiteLaunchFlow({ app, draftState }) {
  ...
  function armConfirmation(kind) {
    ...
    app?.notify?.({
      title: explicitTitleForKind(kind),
      message: explicitMessageForKind(kind),
      tone: 'info',
    });
  }
  ...
  function isConfirmationArmed(kind) {
    return confirmation?.kind === kind;
  }
  ...
  async function confirmHold() {
    ...
    if (!isConfirmationArmed('hold')) {
      armConfirmation('hold');
      return;
    }
  ...
  async function storeRootHtml() {
    ...
    if (!isConfirmationArmed('store')) {
      armConfirmation('store');
      return;
    }
  ...
  async function createSite() {
    ...
    if (!isConfirmationArmed('create')) {
      armConfirmation('create');
      return;
    }
...
function ActiveAction({
  ...
  if (activeStep === 'hold') {
    const armed = confirmation?.kind === 'hold';
    ...
    return (
      <div className="site-launch-action-content">
        ...
        <div className="site-launch-facts-compact">
          ...
          <MiniFact label="Amount ROC" value={formatRocUnits(holdRequest?.amount_minor || '') || 'waiting'} />
          <MiniFact label="Proof" value={paidProof ? 'ready' : holdState.status === 'ok' ? 'incomplete' : 'waiting'} />
        </div>

        {armed && (
          <ExplicitConfirmationPanel
            title="Ready to send ROC hold"
            description="Review this paid action, then click Send ROC Hold. This calls the backend wallet hold path through the configured gateway."
            facts={[
              ['Site', holdRequest ? `crab://${stringValue(holdRequest.memo).replace(/^CrabLink site launch hold for\s*/i, '')}` : 'waiting'],
              ['From', holdRequest?.from || 'waiting'],
              ['Escrow', holdRequest?.to || 'waiting'],
              ['Amount', `${formatRocUnits(holdRequest?.amount_minor || '') || '0'} ROC`],
              ['Nonce', holdRequest?.nonce || 'waiting'],
            ]}
        ...
        <div className="site-page-actions">
          <Button variant="primary" disabled={!holdRequest || holdState.status === 'sending'} onClick={confirmHold}>
            {holdState.status === 'sending' ? 'Holding…' : armed ? 'Send ROC Hold' : 'Review ROC Hold'}
          </Button>
          {armed && (
            <Button variant="secondary" disabled={holdState.status === 'sending'} onClick={clearConfirmation}>
              Cancel
            </Button>
          )}
        </div>
  ...
  if (activeStep === 'store') {
    const armed = confirmation?.kind === 'store';
    ...
    return (
      <div className="site-launch-action-content">
        ...
        {armed && (
          <ExplicitConfirmationPanel
            title="Ready to store root HTML"
            description="Review this storage action, then click Store Root HTML. This sends the root HTML bytes with the wallet hold proof."
            facts={[
              ['Route', '/paid/o'],
              ['Bytes', formatBytes(rootHtmlBytes)],
              ['Txid', paidProof?.txid || 'missing'],
              ['Receipt', paidProof?.receipt_hash || 'missing'],
            ]}
            onCancel={clearConfirmation}
          />
        )}
        ...
        <div className="site-page-actions">
          ...
          >
            {rootStoreState.status === 'sending' ? 'Storing…' : armed ? 'Send Root HTML' : 'Review Root Storage'}
          </Button>
          {armed && (
            <Button variant="secondary" disabled={rootStoreState.status === 'sending'} onClick={clearConfirmation}>
              Cancel
            </Button>
          )}
  ...
  const armed = confirmation?.kind === 'create';
...
function ExplicitConfirmationPanel({ title, description, facts = [], onCancel }) {
  return (
    <div className="site-launch-explicit-confirmation" role="group" aria-label={title}>
      <div>
        <strong>{title}</strong>
        <p>{description}</p>
      </div>

      <div className="site-launch-explicit-grid">
        {facts.map(([label, value]) => (
          <MiniFact key={label} label={label} value={value} monospace={String(value || '').length > 28} />
        ))}
      </div>

      <button type="button" className="site-launch-explicit-cancel" onClick={onCancel}>
