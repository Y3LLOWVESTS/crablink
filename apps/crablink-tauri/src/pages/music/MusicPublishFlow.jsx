/**
 * RO:WHAT — Explicit React prepare → wallet hold → paid music upload flow for crab://music.
 * RO:WHY — Brings bounded music-lite minting to Tauri using the same paid flow proven for video.
 * RO:INTERACTS — musicAssetClient, walletClient, MusicPage, svc-gateway /assets/music/prepare, /wallet/hold, /assets/music.
 * RO:INVARIANTS — no silent ROC spend; rights attestation required; cover art is crab URL reference only; no fake CIDs.
 ...
 * RO:SECURITY — sends bounded audio bytes only to configured svc-gateway; no local file path or cover-art bytes are sent.
 * RO:TEST — manual crab://music prepare/hold/upload smoke from Tauri React shell.
 */
...
import {
  createMusicAssetClient,
  extractMusicAssetCid,
  extractMusicAssetUrl,
  normalizePaidProof,
} from '../../shared/api/musicAssetClient.js';
import {
...
const MAX_SIMPLE_MUSIC_BYTES = 25 * 1024 * 1024;
...
export default function MusicPublishFlow({
  app,
  draft,
  selectedFile,
  fileFacts,
  ...
  const musicClient = useMemo(() => createMusicAssetClient(gateway), [gateway]);
  ...
  const workflowKey = useMemo(
    () =>
      [
        ...
        ownershipReady ? 'rights-ok' : 'rights-missing',
        settings.passportSubject || '',
        settings.walletAccount || '',
      ].join('|'),
    [selectedFile, draft, ownershipReady, settings.passportSubject, settings.walletAccount],
  );
  ...
  const preflight = getPreflight({ selectedFile, settings, gateway, ownershipReady });
  ...
  const preparePayload = useMemo(
    () =>
      selectedFile
        ...
        : null,
    [draft, selectedFile, settings, ownershipReady],
  );
  ...
  const uploadCrabUrl = extractMusicAssetUrl(uploadPayload);
  const uploadAssetCid = extractMusicAssetCid(uploadPayload);
  ...
  async function sendPrepare() {
    ...
    try {
      const response = await musicClient.prepareMusicAsset(preparePayload, {
      ...
      app?.notify?.({
        title: 'Music prepare failed',
        message: error?.message || 'Gateway rejected the music prepare request.',
        tone: 'warning',
      });
    }
  }
  ...
  async function submitUpload() {
    ...
    if (!uploadReviewOpen) {
      ...
      setHoldReviewOpen(false);
      app?.notify?.({
        title: 'Review music upload request',
        message: 'Review the music upload request, then click Send Music Upload. No audio bytes have been uploaded yet.',
        tone: 'info',
      });
      return;
    }
    ...
    const pendingRequest = {
      ...
      rightsAttestation: Boolean(ownershipReady),
    };
    ...
    try {
      const response = await musicClient.uploadMusicAsset({
        ...
        metadata: {
          ...
          coverImageCrabUrl: draft.coverImageCrabUrl || '',
          lyricsCrabUrl: draft.lyricsCrabUrl || '',
          rightsMode: draft.rightsMode || '',
          licenseMode: draft.licenseMode || '',
          legalAttestationAccepted: ownershipReady ? 'true' : '',
        ...
        idempotencyKey:
          ...
          stableIdempotencyKey('music-upload', selectedFile.name, paidProof.txid),
      });

      const data = firstObject(response?.data, response);
      const crabUrl = extractMusicAssetUrl(data);
      const assetCid = extractMusicAssetCid(data);
      ...
      if (crabUrl) {
        writeLocalCatalogEntry({
          ...
          kind: 'music',
          crabUrl,
          title: draft.title || selectedFile?.name || 'Music asset',
          detail: `${formatBytes(selectedFile?.size || 0)} · backend-confirmed music asset`,
          cid: assetCid,
          status: 'backend-confirmed display cache',
          source: 'music_upload_success',
      ...
      app?.notify?.({
        ...
        message: error?.message || 'Gateway rejected the music upload request.',
        tone: 'danger',
      });
    }
  }
  ...
  return (
    <Card
      eyebrow="Mint"
      title="Paid music mint flow"
      className="cl-music-publish-card"
      actions={
        <div className="cl-music-publish-badges">
    ...
    >
      <p className="cl-music-publish-copy">
        This is the music-lite version of the proven video workflow. Prepare is pricing/policy
        preflight. The ROC hold and audio byte upload are separate explicit clicks. Cover art is only
        a crab:// image reference; no cover-art bytes are uploaded from this page.
      </p>

      <section className="cl-music-publish-grid" aria-label="Music publish prerequisites">
        ...
        <Fact label="Music-lite cap" value={formatBytes(MAX_SIMPLE_MUSIC_BYTES)} />
        ...
        <Fact label="Rights gate" value={ownershipReady ? 'attested' : 'required'} />
      </section>

      {!preflight.ok && (
        <div className="cl-music-publish-warning">
          <strong>Mint flow blocked</strong>
          <span>{preflight.reason}</span>
        </div>
      )}

      <section className="cl-music-publish-step">
        <header>
          <div>
            <p className="cl-eyebrow">Step 1</p>
            <h3>Prepare music upload</h3>
          </div>
          <Badge tone={toneForStatus(prepareState.status)}>{labelForStatus(prepareState.status)}</Badge>
        </header>

        <p>
          Sends strict JSON metadata to <code>/assets/music/prepare</code>. It does not upload audio
          bytes and does not mutate the wallet.
        </p>

        <div className="cl-music-publish-actions">
      ...
      <section className="cl-music-publish-step">
        ...
        <div className="cl-music-publish-controls">
        ...
        <div className="cl-music-publish-grid">
        ...
        {holdReviewOpen && holdApiRequest && (
          <div className="cl-music-publish-warning" role="status" aria-live="polite">
            <strong>Review ROC hold for music upload</strong>
            <span>
      ...
      <section className="cl-music-publish-step">
        <header>
          <div>
            ...
            <h3>Review and submit music upload</h3>
          </div>
          <Badge tone={toneForStatus(uploadState.status)}>{labelForStatus(uploadState.status)}</Badge>
        </header>

        <p>
          Sends the selected audio bytes as the raw request body to <code>/assets/music</code> with
          wallet hold proof headers. Cover art remains a referenced crab:// image asset.
        </p>

        {uploadReviewOpen && (
          <div className="cl-music-publish-warning" role="status" aria-live="polite">
            <strong>Review music upload request</strong>
            <span>
              File: {selectedFile?.name || 'selected audio'} · Bytes: {selectedFile?.size || 0} · Content-Type: {selectedFile?.type || inferAudioContentType(selectedFile?.name)}
            </span>
            <span>
              No audio bytes have been uploaded yet. Click <strong>Send Music Upload</strong> to submit the raw bytes and paid proof.
            </span>
          </div>
        )}
        ...
        <div className="cl-music-publish-actions">
          <Button variant={uploadReviewOpen ? 'primary' : 'secondary'} disabled={!canUpload || uploadState.status === 'sending'} onClick={submitUpload}>
            {uploadState.status === 'sending' ? 'Uploading…' : uploadReviewOpen ? 'Send Music Upload' : 'Review Music Upload'}
          </Button>
          {uploadReviewOpen && (
        ...
        {uploadState.status === 'ok' && (
          <div className="cl-music-upload-success">
...
function getPreflight({ selectedFile, settings, gateway, ownershipReady }) {
  if (!gateway) {
    return {
      ok: false,
  ...
  if (!selectedFile) {
    return {
      ok: false,
      reason: 'Choose an audio file before preparing a paid music upload.',
    };
  }
  ...
  if (!isAudioFile(selectedFile)) {
    return {
      ok: false,
      reason: 'Choose a valid audio file for this music-lite mint path.',
    };
  }

  if (Number(selectedFile.size || 0) > MAX_SIMPLE_MUSIC_BYTES) {
    return {
      ok: false,
      reason: `This music-lite upload path is capped at ${formatBytes(MAX_SIMPLE_MUSIC_BYTES)}. Larger media needs the future bounded range/segment path.`,
    };
  }
  ...
  if (!ownershipReady) {
    return {
      ok: false,
      reason: 'Complete the music rights attestation before minting.',
    };
  }
  ...
  if (!String(settings?.walletAccount || '').trim()) {
    return {
      ok: false,
      reason: 'Configure a wallet account label before preparing a music upload.',
    };
  }
  ...
  if (!String(settings?.passportSubject || '').trim()) {
    return {
      ok: false,
      reason: 'Configure a passport subject before preparing a music upload.',
    };
  }
...
function buildPreparePayload({ draft, selectedFile, settings, ownershipReady }) {
...
function buildHoldRequest({ amountMinor, escrowAccount, holdNonce, prepareData, preparePayload, settings, selectedFile }) {
  ...
  return {
    ...
    amount_minor: amount,
    nonce: safeNonce,
    memo: `CrabLink music hold for ${selectedFile?.name || preparePayload?.title || 'music upload'}`.slice(0, 240),
...
function buildWalletHoldIdempotencyKey({ preparePayload, from, to, amountMinor, nonce }) {
  return compactIdempotencyKey(
    stableIdempotencyKey(
      'wallet-hold',
      preparePayload?.client_idempotency_key || 'music-prepare',
      from,
      to,
      amountMinor,
      nonce,
...
function Fact({ label, value, monospace = false }) {
  return (
    <div className="cl-music-publish-fact">
      <span>{label}</span>
