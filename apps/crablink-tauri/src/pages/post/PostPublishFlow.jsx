 ...
 * RO:WHY — Starts the post asset publishing lane without faking backend truth; Tauri uses in-page review/send instead of window.confirm.
 ...
 * RO:TEST — manual crab://post prepare/hold/publish smoke from Tauri React shell after backend post routes exist.
 */
...
export default function PostPublishFlow({ app, draftState }) {
  ...
  async function submitPublish() {
    ...
    if (!publishReviewOpen) {
      ...
      app?.notify?.({
        title: 'Review post publish',
        message: 'Review the publish request, then click Send Post Publish. No post JSON has been sent yet.',
        tone: 'info',
      });
  ...
  return (
    ...
    >
      <p className="post-section-copy">
        This is the gateway-wired lane for the post primitive. It can call the expected
      ...
      <section className="post-publish-step">
        ...
        {holdReviewOpen && (
          <div className="post-publish-warning">
            <strong>Review before sending ROC hold</strong>
            <span>
              No ROC has been sent yet. Click <strong>Send ROC Hold</strong> to submit this wallet request through the gateway.
            </span>
          </div>
        )}
      ...
      <section className="post-publish-step">
        <header>
          <div>
            ...
            <h3>Review and submit post publish</h3>
          </div>
        ...
        {publishReviewOpen && (
          <div className="post-publish-warning">
            <strong>Review before publishing post</strong>
            <span>
              No post JSON has been sent yet. Click <strong>Send Post Publish</strong> to submit the post and paid proof.
            </span>
          </div>
        )}
        ...
        <div className="post-publish-actions">
          <Button variant={publishReviewOpen ? 'primary' : 'secondary'} disabled={!canPublish || publishState.status === 'sending'} onClick={submitPublish}>
            {publishState.status === 'sending' ? 'Publishing…' : publishReviewOpen ? 'Send Post Publish' : 'Review Post Publish'}
          </Button>
          {publishReviewOpen && (
