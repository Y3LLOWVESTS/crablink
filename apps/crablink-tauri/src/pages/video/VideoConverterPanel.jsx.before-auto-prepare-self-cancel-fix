...
export default function VideoConverterPanel({ selectedFileFacts, playbackMeta, draft, initialMakeExportHandoff = null, initialSourceHandle = '', onPrepareJobUpdate = null }) {
  ...
  useEffect(() => {
    ...
    const safeJob = {
      ...state.job,
      crabLinkVideoSessionId: converterSessionId,
      crabLinkVideoSessionStartedAtUnixMs: converterSessionStartedAtMs.current,
      crabLinkVideoPreparedAtUnixMs: Date.now(),
      crabLinkVideoSourceFingerprint: sourceFingerprintForFacts(effectiveSourceFacts),
      truthBoundary: {
...
function sourceFingerprintForFacts(facts = {}) {
  const clean = {
    fileName: stringValue(facts.fileName, facts.name),
    contentType: stringValue(facts.contentType, facts.type),
    bytes: positiveInteger(facts.bytes, facts.size),
    durationSeconds: positiveNumber(facts.durationSeconds, facts.duration_seconds),
    width: positiveInteger(facts.width),
    height: positiveInteger(facts.height),
    source: stringValue(facts.source),
  };
