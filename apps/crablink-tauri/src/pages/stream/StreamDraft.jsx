 ...
 * RO:INTERACTS — StreamPage, StreamPricingPanel, StreamPodcastMode, stream.css, future stream/session APIs.
 ...
 * RO:SECURITY — no stream keys, ingest tokens, private URLs, local paths, or spend authority are collected here.
...
const SOURCE_OPTIONS = Object.freeze([
  ['local_camera_or_screen_preview', 'Local camera/screen preview'],
  ['local_camera_preview', 'Local camera preview active'],
  ['local_screen_preview', 'Local screen preview active'],
...
const INGEST_OPTIONS = Object.freeze([
  ['not_wired_local_preview', 'Not wired / local preview only'],
...
const ACCESS_OPTIONS = Object.freeze([
  ['paid_interval_manual_renew', 'Paid interval / manual renew'],
  ['public_preview_paid_interval', 'Public preview + paid interval'],
...
const CHAT_OPTIONS = Object.freeze([
  ['chat_placeholder_future', 'Chat placeholder / later'],
...
const AUDIO_OPTIONS = Object.freeze([
  ['off', 'Off for local preview'],
  ['on', 'Ask for audio permission'],
]);
    ...
    <form className="cl-stream-form" onSubmit={(event) => event.preventDefault()}>
      ...
      <section className="cl-stream-subpanel" aria-label="Schedule and source">
        <div className="cl-stream-subhead">
          ...
          <h3>Control-room planning</h3>
          <p>
            Local camera/screen preview is allowed here. Backend stream sessions, ingest tokens, and
            playable viewer streams are still not wired.
          </p>
        </div>
        ...
        <div className="cl-stream-three">
          ...
          <Field label="Local audio preview">
            <select
              value={draft.captureAudio}
              onChange={(event) => updateField('captureAudio', event.target.value)}
            >
      ...
      <section className="cl-stream-subpanel" aria-label="Access and moderation">
        ...
        <div className="cl-stream-builder-note">
          ...
          <span>
            {stats.tags.length} tags · {stats.linkedAssetCount} linked references · {stats.pricing.summary}.
            This route intentionally stops at local control-room design until backend stream/session routes exist.
          </span>
        </div>
      </section>
    </form>
