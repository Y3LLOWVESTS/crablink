 ...
 * RO:WHY — Ports root HTML storage proof into the Tauri React lane without WebView/CORS upload failure.
...
export class ObjectClient {
  ...
  async uploadPaidObject({
    ...
    const uploadBody = normalizeUploadBody(bytes, contentType);
    const proof = normalizePaidProof(paidProof);
    const idem =
      idempotencyKey ||
      stableObjectIdempotencyKey('paid-object', proof.txid, String(uploadBody.size), objectKind);
...
function normalizeUploadBody(bytes, contentType) {
