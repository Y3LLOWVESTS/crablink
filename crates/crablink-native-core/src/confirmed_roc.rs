...
//! RO:INTERACTS — Phase 22G exported projection, desktop compatibility adapter, and future TV display validation.
...
//! RO:TEST — module unit tests plus the desktop Phase 22 projection integration test.
...
fn validate_b3(field: &str, value: &str) -> Result<(), String> {
    validate_canonical_b3(value).map_err(|_| format!("{field} must be b3:<64 lowercase hex>",))
}
