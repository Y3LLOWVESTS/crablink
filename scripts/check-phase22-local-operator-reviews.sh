#!/usr/bin/env bash
# RO:WHAT — Live Phase 22C runner for CrabLink moderation and persistence review.
# RO:WHY — Proves production Tauri review commands against real independent node state.
# RO:INVARIANTS — loopback-only; ephemeral auth; metadata-only decisions; no economic or finality authority.
# RO:TEST — invoked by npm run check:phase22-local-operator-reviews.
...
require_file() {
  ...
  if [[ ! -f "$file" ]]; then
    echo "required Phase 22C source missing: $file" >&2
    exit 1
  fi
}
...
require_file \
  "$TAURI_ROOT/src-tauri/tests/phase22_live_operator_reviews.rs"
...
TMP_DIR="$(
  mktemp -d \
    "${TMPDIR:-/tmp}/crablink-phase22c.XXXXXX"
)"
...
cleanup() {
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    echo >&2
    echo "Phase 22C failed. Recent macronode log:" >&2
    tail -n 80 "$MACRONODE_LOG" 2>/dev/null >&2 || true

    echo >&2
    echo "Recent micronode log:" >&2
    tail -n 80 "$MICRONODE_LOG" 2>/dev/null >&2 || true
  fi
...
echo
echo "== Phase 22C: build independent node binaries =="
echo
...
echo
echo "== Phase 22C: run live review workflow =="
echo
...
cargo test \
  ...
  --test phase22_live_operator_reviews \
  phase22_crablink_reviews_real_runtime_metadata \
  -- \
  --ignored \
  --nocapture
...
echo
echo "Phase 22C live Operator review passed."
echo "Moderation and persistence queues used real runtime state."
echo "The User Node remained active and independently managed."
echo "No policy activation, durable byte write, provider mutation,"
echo "reward finality, wallet mutation, ledger mutation, or"
echo "confirmed ROC was created."
