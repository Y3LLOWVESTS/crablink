...
# RO:WHY — Keeps Tauri-first migration and QuickChain client-boundary checks reproducible while preserving macOS media dev config.
# RO:INTERACTS — apps/crablink-tauri/package.json, Vite, Tauri Rust crate, macOS media config, QuickChain boundary scanners.
# RO:INVARIANTS — no app launch; no wallet mutation; no gateway mutation; no client QuickChain authority; Cargo check must match macos-private-api config.
...
test -d "$APP" || { echo "missing apps/crablink-tauri; run scripts/scaffold_crablink_tauri.sh first" >&2; exit 1; }
cd "$APP"
